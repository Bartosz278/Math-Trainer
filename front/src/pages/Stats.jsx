...
function Stats() {
  ...
  const allQuestions =
    stats?.reduce((acc, current) => acc + current.totalQuestions, 0) || 0;
  const mistakes =
    stats?.reduce((acc, current) => acc + current.wrongAnswers, 0) || 0;
    ...
    <div className="relative h-full">
      ...
      <div className="bg-gray-400/70 font-mono mx-auto border-[5px] rounded-[20px] drop-shadow-[0_35px_35px_rgba(0,0,0,0.8)] md:w-2/3 w-[80%] max-w-[500px] h-5/6 mt-8 p-6 flex flex-col text-white z-10 text-2xl">
          ...
          <ol className="overflow-auto">
            {stats.map((game, i) => (
              <StatsRow
                ...
                stats={game}
                id={game.id || i}
                isExpanded={expandedId === (game.id || i)}
        ...
        <div className="flex flex-col items-center">
          <p className="mt-3 text-md mx-auto p-3 text-white">
            Back to main menu
          </p>
          <Link to="/">
